...
// - '$5' -> ssh_port
// - '$6' -> download_site
var install_script = replace(replace(replace(replace(replace(replace(loadTextContent('install.sh'), 'lustre_mgs="$1"', 'lustre_mgs="${lustre_mgs}"'), 'storage_account="$2"', 'storage_account="${storage_account_name}"'), 'storage_sas="$3"', 'storage_sas="${storage_sas_key}"'), 'storage_container="$4"', 'storage_container="${storage_container_name}"'), 'ssh_port="$5"', 'ssh_port="${ssh_port}"'), 'download_url="$6"', 'download_url="${download_url}"')
